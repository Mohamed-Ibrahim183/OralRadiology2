...
class GROUP
{
  ...
  public function Update($postKeys)
  {
    $FrontData = $postKeys;
    unset($FrontData['Name']);


    $lastGroupID = $FrontData["GroupId"];
    // print_r();
    print_r(json_decode($FrontData["Slots"], true));


    // 1.1 get the slots ids to delete it
    $stmt = $this->pdo->prepare("SELECT SlotId from groupsslots where GroupId=:selected");
    $stmt->execute([":selected" => $lastGroupID]);
    $slotsIds = $stmt->fetchAll();
    // 2. Delete each slot from slots table
    // $this->pdo->beginTransaction();

    foreach ($slotsIds as $slotId) {
      $stmt = $this->pdo->prepare("DELETE from slots where Id=:selected");
      $stmt->execute([":selected" => $slotId['SlotId']]);
    }

    foreach (json_decode($FrontData["Slots"], true) as  $SLOT) {
      $condition = true;
      print_r($SLOT);
      ...
      // }
      if ($condition === true) {
        // 1. delete slots from its table


        // 2. Save Each New Slot
        // $secondArray = json_decode($value, true);
        $this->prepareAndBind("INSERT into Slots (Day, StartTime, EndTime, Room) VALUES", [
          "Day" => htmlspecialchars(trim($SLOT["day"])),
          "StartTime" => htmlspecialchars(trim($SLOT["Start"])),
          "EndTime" => htmlspecialchars(trim($SLOT["End"])),
          "Room" => htmlspecialchars(trim($SLOT["Room"]))
        ], true);
        $lastSlotID = $this->pdo->lastInsertId();
